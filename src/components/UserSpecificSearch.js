...
function KeywordForm() {
    ...
    useEffect(()=> {
        ...
        selector.keywords = keys;
        async function f1() {
            setLoading(true);

            axios.post('http://127.0.0.1:9000/query/', selector)
            .then((res) => {
                setKeys(res.data);
                document.getElementById("SearchButton").click();
                document.getElementById("SearchBox").value = res.data;
            })
    ...
    return (
        <Container size="xl" style={{ height: '100%', width: '100%' }} >
            ...
            {
                active ?
                    <List keywords={keys} data={data} selector={selector} setSelector={setSelector} /> :
                    loading ?
